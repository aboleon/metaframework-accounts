...
<table class="table table-striped table-bordered dt-responsive nowrap table-hover" cellspacing="0" width="100%">
    ...
    <thead>
        <tr>
            <th>{{ __('ui.Name') }}</th>
            <th>{{ trans_choice('ui.description', 2) }}</th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr class="hover">
            ...
            <td>
                ...
                <x-mfw::modal :route="route('mfw-accounts.pay-mean-channels.destroy', $item)"
                              title="{{ __('mfw.delete') }}"
                              question="{!! __('mfw.should_i_delete_record') !!} - <b>{{ $item->id . ' '. ($item->translation->name ?? '-') }}</b>"
                              reference="remove_paymeanschannel_{{ $item->id }}"
